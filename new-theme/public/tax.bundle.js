 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:j}=window,B=function(t,e){j.post(t).then((()=>window.location.assign(e)))},{$:A}=window;class T{extend(t){t.getContainer().on("click",p,(t=>{B(A(t.currentTarget).data("url"),A(t.currentTarget).data("redirect"))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Pt}=window;Pt((()=>{const e=new x("tax");e.addExtension(new jt),e.addExtension(new M),e.addExtension(new O),e.addExtension(new T),e.addExtension(new $),e.addExtension(new Ct),e.addExtension(new xt),e.addExtension(new Lt),e.addExtension(new Mt),e.addExtension(new $t),new Ft,new Tt,window.prestashop.component.initComponents(["MultistoreConfigField"]),Pt(t.useEcoTax).on("change",(e=>{const n=Number(Pt(e.currentTarget).val());Pt(t.rowEcoTaxRuleGroup).toggleClass("d-none",0===n)}))}))})(),window.tax=o})();