...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    mounted() {
      ...
      let _this = this;
      $(document).on("file-robot-upload-url", function() {
        _this.resetDropzone();
      });
    },
    methods: {
      ...
      async initProductImages(): Promise<void> {
        try {
          ...
          this.loading = false;
          this.initDropZone();
          const callback = undefined; // Optional callback when it's done
          const crossOrigin = 'anonymous'; // Added to the `img` tag for crossOrigin handling
          images.forEach((image: Dropzone.DropzoneMockFile) => {
            this.dropzone?.displayExistingFile(image, image.image_url, callback, crossOrigin);
          });
        } catch (error) {
