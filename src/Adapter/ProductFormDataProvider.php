...
class ProductFormDataProvider implements FormDataProviderInterface
{
    ...
    {
        $cover_thumbnail = \Db::getInstance()->executeS(
            '
            SELECT image_shop.`cover`, i.`id_image`, il.`legend`, i.`position`, i.`url`
            FROM `' . _DB_PREFIX_ . 'image` i
            ' . \Shop::addSqlAssociation('image', 'i') . '
             LEFT JOIN `' . _DB_PREFIX_ . 'image_lang` il ON (i.`id_image` = il.`id_image`)
            WHERE i.`cover` = 1 AND i.`cover` AND i.`url` IS NOT NULL AND i.`id_product` = ' . (int) $productForEditing->getProductId()
        );
        return [
            ...
            'cover_thumbnail' => count($cover_thumbnail) > 0 ? $cover_thumbnail[0]['url'] :$productForEditing->getCoverThumbnailUrl(),
