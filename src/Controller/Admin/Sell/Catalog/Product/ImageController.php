...
namespace Scaleflex\PrestashopFilerobot\Controller\Admin\Sell\Catalog\Product;
...
class ImageController extends FrameworkBundleAdminController
{
    ...
    public function getImagesForShopAction(int $productId, int $shopId): JsonResponse
    {
        ...
        $filerobot_images = \Db::getInstance()->executeS(
            '
        ...
        $filerobot_images = array_column($filerobot_images, 'url', 'id_image');

        $images = array_map([$this, 'formatImage'], $images);

        foreach($images as $key => $image){
            if (array_key_exists($image['image_id'], $filerobot_images) && $filerobot_images[$image['image_id']]) {
                $image['image_url'] = $filerobot_images[$image['image_id']];
                $image['thumbnail_url'] = $filerobot_images[$image['image_id']];
                $images[$key] = $image;
            }
        }
